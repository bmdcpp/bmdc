/*
 * Copyright © 2009-2014 freedcpp, http://code.google.com/p/freedcpp
 *
...
void SearchSpy::preferences_gui()
{
	...
	for (UnMapIter::const_iterator it = searchIters.begin(); it != searchIters.end(); ++it)
	{
		iter = it->second;
...
{
	UnMapIter::const_iterator it = searchIters.find(search);
...
void SearchSpy::on(TimerManagerListener::Minute, uint32_t tick) noexcept
{
