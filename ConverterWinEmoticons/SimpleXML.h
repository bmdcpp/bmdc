...
{
public:
	SimpleXML() : root("BOGUSROOT", "", NULL), current(&root), found(false) {
		resetCurrentChild();
	}
	~SimpleXML() { }
	...
	const string& getChildAttrib(const string& aName, const string& aDefault = "") {
		checkChildSelected();
...
private:
	...
	public:
		...
		const string& getAttrib(const string& aName, const string& aDefault = "") {
