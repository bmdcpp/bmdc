...
bool CryptoManager::TLSOk() const noexcept {
    return /*SETTING(USE_TLS)*/ true && certsLoaded && !keyprint.empty();
}
...
void CryptoManager::loadCertificates() noexcept {
    if(!/*SETTING(USE_TLS)*/true || !clientContext || !clientVerContext || !serverContext || !serverVerContext)
        return;
    ...
    if(File::getSize(cert) == -1 || File::getSize(key) == -1 || !checkCertificate()) {
        ...
        try {
            ...
            LogManager::getInstance()->message(autosprintf("TLS disabled, failed to generate certificate: %s",e.getError().c_str()));
        }
    }
...
string CryptoManager::keySubst(const uint8_t* aKey, size_t len, size_t n) {
    uint8_t* temp = new uint8_t[len + n * 10];
...
string CryptoManager::makeKey(const string& aLock) {
    ...
    uint8_t* temp = new uint8_t[aLock.length()];
    uint8_t v1;
    size_t extra=0;
