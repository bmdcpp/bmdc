...
void PluginManager::unloadPlugins() {
	...
	// Off we go...
	for(auto plugin = plugins.end();plugin!= plugins.begin(); plugin--) {
		if((*plugin).handle) {
...
bool PluginManager::onUDP(bool out, const string& ip, const string& port, const string& data) {
	UDPData udp = { ip.c_str(), (unsigned short)Util::toUInt(port) };
...
void PluginManager::enable(Plugin& plugin, bool install, bool runtime) {
	...
	if(!plugin.handle) {
		char buf[1024];
		sprintf(buf,_("Error loading %s: %s") , plugin.name.c_str() , GET_ERROR());
		throw Exception(buf);
	}
	bool unload = true;
	...
	if(info.apiVersion != DCAPI_CORE_VER) {
		char buf[1024];
		sprintf(buf,_("%s is not compatible with %s") , info.name , APPNAME);
		throw Exception(buf);
	}
	...
	for(decltype(info.numDependencies) i = 0; i < info.numDependencies; ++i) {
		if(!isLoaded(info.dependencies[i])) {
			char buf[1024];
			sprintf(buf,_("Missing dependencies for %s") , info.name);
			throw Exception(buf);
		}
	}
	...
	if(install) {
		...
		if(dupe) {
			...
			if(dupe->version < info.version) {
				...
				char buf[1024];
				sprintf(buf,_("Updating the %s plugin from version %f to version %f"),dupe->name.c_str(),dupe->version,info.version);
				...
				char buf[1024];
				sprintf(buf,_("%s is already installed"), info.name);
				throw Exception(buf);
			}
	...
	{ // scope to change the plugin ref
		...
		if(plugin.dcMain(install ? ON_INSTALL : runtime ? ON_LOAD_RUNTIME : ON_LOAD, &dcCore, nullptr) == False) {
			...
			char buf[1024];
            sprintf(buf, _("Error loading %s"), plugin.name.c_str());
			throw Exception(buf);
		}
