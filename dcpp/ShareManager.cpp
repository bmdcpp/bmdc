	...
	if(forceXmlRefresh || (xmlDirty && (lastXmlUpdate + 15 * 60 * 1000 < GET_TICK() || lastXmlUpdate < lastFullUpdate))) {
		...
		try {
			...
			if(!Util::fileExists(Util::getPath(Util::PATH_USER_CONFIG) + "Emptyfiles.xml.bz2")) {
					...
					emptyXmlFile.write("<FileListing Version=\"1\" CID=\"" + ClientManager::getInstance()->getMe()->getCID().toBase32() + "\" Base=\"/\" Generator=\"DC++ " VERSIONSTRING "\">\r\n");
		...
		for(StringMapIter i = shares.begin(); i != shares.end(); i++) {
			if(Util::strnicmp(i->first, n, i->first.size()) == 0 && n[i->first.size() - 1] == PATH_SEPARATOR) {
				try {
					...
					HashManager::getInstance()->checkTTH(n, File::getSize(n)/*qi->getSize()*/, 0);
				} catch(const Exception&) {
					// Not a vital feature...
				}
				break;
