        ...
        local fnc, err = loadfile( DC():GetConfigPath().."scripts//libsimplepickle.lua" )
        if fnc then
           fnc()
           fnc = nil
        else
        ...
        scriptMan.Init = function( filename )
           ...
           setmetatable( scriptMan.script, { __index = {
                  load = function( this, filename )
                         ...
                         local func, err = loadfile( DC():GetConfigPath().."scripts//"..filename )
                  ...
                  save = function( this )
                         pickle.store( DC():GetConfigPath().."scripts//scriptlist.tbl", { scriptlist = scriptlist } )
                  end}
           })
        ...
        if not scriptlist then
           local func = loadfile( DC():GetConfigPath() .. "scripts//scriptlist.tbl" )
           if func then
                  func()
           else
                  local f = io.open( DC():GetConfigPath() .. "scripts//scriptlist.tbl", "w+")
